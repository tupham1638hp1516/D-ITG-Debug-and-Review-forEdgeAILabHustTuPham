...
FILE *intxt;
FILE *outtxt;
int itg_size, flagfilter, flagwinlin;
...
if (argv[1] != NULL) strncpy(loginname, argv[1], fileNameSize);
	argv += 2;
	argc -= 2;
	while (argc > 0) {
		if (argv[0][0] == '-') {
...
struct info *readline(ifstream *fbin, FILE *ftxt, int filetype)
{
	...
	if (filetype == 0) {
		...
		itg_size = fread(infos_loc, sizeof(struct info), 1, ftxt); 
	...
	while (memFile[i].flowId != (unsigned int) -1) {
		...
		if (inputfilec == 0) {
			...
			while (1) { 
				...
				itg_size = fread(infos, sizeof(struct info), 1, inputfilec); 
